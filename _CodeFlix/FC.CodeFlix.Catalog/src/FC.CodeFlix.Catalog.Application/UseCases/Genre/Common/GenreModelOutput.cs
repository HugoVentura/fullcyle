...
namespace FC.CodeFlix.Catalog.Application.UseCases.Genre.Common
{
    public class GenreModelOutput
    {
        ...
        public static GenreModelOutput FromGenre(DomainEntity.Genre genre) => new(genre.Id, genre.Name, genre.IsActive, genre.CreatedAt, 
            genre.Categories.Select(categoryId => new GenreModelOutputCategory(categoryId)).ToList().AsReadOnly());
    }
    ...
    public class GenreModelOutputCategory
    {
        ...
        public GenreModelOutputCategory(Guid id, string? name = null) => (this.Id, this.Name) = (id, name);
    }
}
